...
namespace Klyte.Commons.Utils
{
    public static class TextureFilter
    {
        ...
        #region Grayscale
        public static Texture2D Grayscale(Texture2D inTex) => Filter(inTex, GrayscaleFunc);
        ...
        #region Threshold
        public static Texture2D Threshold(Texture2D inTex, float threshold) => Filter(inTex, ThresholdFunc, threshold);
        ...
        public static float[][] Normalized(this float[][] kernel) => Normalize(kernel);
        ...
        public static Color[] Convolution(int width, int height, Color[] inPixels, float[][] kernel, int iteration) => ApplyFilter(width, height, ConvolutionFunc, Time.realtimeSinceStartup, inPixels, kernel, iteration);
        ...
        #region Sepia
        public static Texture2D Sepia(Texture2D inTex) => Filter(inTex, SepiaFunc);
        ...
        private static void SepiaFunc(int width, int height, Color[] inPixels, ref Color[] outPixels, params object[] parameters)
        {
            for (int i = 0; i < inPixels.Length; i++)
            {
                ...
                var sepiaConvert = new Vector3(0.191f, -0.054f, -0.221f);
        ...
        public static Texture2D ContrasteSaturationBrightness(Texture2D inTex, float contrast, float saturation, float brightness) => Filter(inTex, ContrasteSaturationBrightnessFunc, saturation, brightness, contrast);
        ...
        private static void ContrasteSaturationBrightnessFunc(int width, int height, Color[] inPixels, ref Color[] outPixels, params object[] parameters)
        {
            ...
            for (int i = 0; i < inPixels.Length; i++)
            {
                ...
                var LuminaceCoeff = new Vector3(0.2125f, 0.7154f, 0.0721f);

                //Brigntess calculations
                var AvgLumin = new Vector3(AvgLumR, AvgLumG, AvgLumB);
                Vector3 brtColor = new Vector3(color.r, color.g, color.b) * brightness;
                float intensityf = Vector3.Dot(brtColor, LuminaceCoeff);
                var intensity = new Vector3(intensityf, intensityf, intensityf);

                //Saturation calculation
                var satColor = Vector3.Lerp(intensity, brtColor, saturation);

                //Contrast calculations
                var conColor = Vector3.Lerp(AvgLumin, satColor, contrast);
        ...
        #region SobelFilter
        public static Texture2D SobelFilter(Texture2D inTex) => Filter(inTex, SobelFilterFunc);
        public static Color[] SobelFilter(int width, int height, Color[] inPixels) => ApplyFilter(width, height, SobelFilterFunc, Time.realtimeSinceStartup, inPixels);
